...
if start_date > end_date:
    ...
    if page == "Ticker Predicter Dashboard": 
        ...
        top_6_symbols = ['AAPL', 'GOOGL', 'AMZN', 'MSFT', 'META']

        # Function to get daily returns for a specific stock
        def get_daily_returns(symbol, start, end):
            data = yf.download(symbol, start=start, end=end)
            daily_returns = data['Close'].pct_change().dropna()
            return daily_returns
        
        
       # Create a Plotly figure for daily returns of top 6 companies
        figreturn = go.Figure()

        for symbol in top_6_symbols:
            daily_returns = get_daily_returns(symbol, start_date_str, end_date_str)
            daily_returns.index = pd.to_datetime(daily_returns.index)
            
            figreturn.add_trace(go.Scatter(x=daily_returns.index, y=daily_returns, mode='lines', name=symbol))

        figreturn.update_layout(title='Top Stock Companies Daily Returns for past 3 Months', xaxis_title='Date', yaxis_title='Daily Return')
        ...
        #figheat.update_layout(autosize=True, width=550, height=450)
        


        
        
        #Two columns for Returns chart and RSI chart
        ...
        with co14:
            con14 = st.container(border=True)
    ...
    # Page: Stock Charts
        ...
        fig_moving_avg = px.line(data, x=data.index, y='Close', title=f'Stock Closing Price with Moving Averages for {symbol}')
        fig_moving_avg.add_scatter(x=data.index, y=data['Close'].rolling(window=10).mean(), mode='lines', name='10-Day Moving Average')
        fig_moving_avg.add_scatter(x=data.index, y=data['Close'].rolling(window=30).mean(), mode='lines', name='30-Day Moving Average')
        fig_moving_avg.add_scatter(x=data.index, y=data['Close'].rolling(window=50).mean(), mode='lines', name='50-Day Moving Average')
